    ...
    <section className="surface rounded-3xl p-6">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <button
          ...
          className="relative overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 via-blue-500 to-indigo-600 bg-[length:200%_auto] px-6 py-3 font-bold text-white shadow-lg transition-all hover:scale-[1.02] hover:shadow-indigo-500/30 disabled:cursor-not-allowed disabled:opacity-50 animate-[gradient_3s_linear_infinite]"
        >
          {loading && !isDefaultText ? (
            <span className="flex items-center gap-2">
              <svg className="h-4 w-4 animate-spin" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
              {isEn ? "Generating..." : "生成中..."}
            </span>
          ) : (
            <span className="flex items-center gap-2">
